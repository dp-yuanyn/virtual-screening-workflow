...
def calc_torsion_tree_info(input_file:str, results_dir:str):
    out_file = os.path.join(results_dir, os.path.basename(input_file))
    mol = Chem.SDMolSupplier(input_file, removeHs=False)[0]
    if not mol:
        print(f"ligand {input_file} invalid")
        return
    try:
        runner = TopologyBuilder(mol)
        runner.build_molecular_graph()
        runner.write_sdf_file(out_file)
    except:
        print(f"file {input_file} calc torsion tree err: {traceback.format_exc()}")


def get_torsion_num_from_content(content:str) -> int:
    tag = False
    count = 0
    for line in content.split("\n"):
        if line.startswith(">  <torsionInfo>"):
            tag = True
        elif tag and not line.strip():
            break
        elif tag:
            count += 1
    return count
...
def chunk_iterable_by_torsion_list(mols:list[object], torsion_num_list:list):
    import math

    assert len(mols) == len(torsion_num_list), "mols and torsion_num_list must have the same length"

    torsion_num_threshold_list = [6, 12]
    torsion_num_keep_threshold_list = [4000, 1000]
    batch_size_list = [18000, 10000, 4000]

    normal_ind = [i for i, num in enumerate(torsion_num_list) if num < torsion_num_threshold_list[0]]
    normal_mols = [mols[i] for i in normal_ind]
    
    medium_ind = [i for i, num in enumerate(torsion_num_list) if 
                  num >= torsion_num_threshold_list[0] and 
                  num <= torsion_num_threshold_list[1]]
    medium_mols = [mols[i] for i in medium_ind]
    if len(medium_mols) < torsion_num_keep_threshold_list[0]:
        normal_mols.extend(medium_mols)
        medium_mols = []
    
    heavy_ind = [i for i, num in enumerate(torsion_num_list) if num > torsion_num_threshold_list[1]]
    heavy_mols = [mols[i] for i in heavy_ind]
    if len(heavy_mols) < torsion_num_keep_threshold_list[1]:
        normal_mols.extend(heavy_mols)
        heavy_mols = []

    chunked_mols = []
    for i, one_type_mols in enumerate([normal_mols, medium_mols, heavy_mols]):
        batch_size = batch_size_list[i]
        if one_type_mols:
            real_batch_size = math.ceil(len(one_type_mols) / math.ceil(len(one_type_mols) / batch_size))
            chunked_mols.extend([one_type_mols[j:j+real_batch_size] for j in range(0, len(one_type_mols), real_batch_size)])
